...
knowledgeRouter.put('/sources/:id', (req: Request, res: Response) => {
  const src = db.prepare('SELECT * FROM knowledge_sources WHERE id = ?').get(req.params.id)
  if (!src) {
...
knowledgeRouter.delete('/sources/:id', (req: Request, res: Response) => {
  ...
  const images = db.prepare('SELECT stored_name FROM knowledge_images WHERE source_id = ?').all(src.id) as {
    stored_name: string
  }[]
  for (const img of images) {
    const p = path.join(KNOWLEDGE_IMAGES_DIR, img.stored_name)
...
knowledgeRouter.get('/items', (req: Request, res: Response) => {
  ...
  if (mastery !== undefined && MASTERY_LEVELS.includes(Number(mastery))) {
...
knowledgeRouter.post('/images', imgUpload.single('file'), (req: Request, res: Response) => {
  const sourceId = Number(req.body?.source_id)
  if (!sourceId || !db.prepare('SELECT id FROM knowledge_sources WHERE id = ?').get(sourceId)) {
    res.status(422).json({ message: '面经不存在' })
    return
  ...
  if (!req.file) {
    res.status(422).json({ message: '请选择图片' })
    return
  }
  const result = db
    .prepare('INSERT INTO knowledge_images (source_id, filename, stored_name, created_at) VALUES (?, ?, ?, ?)')
    .run(sourceId, fixOriginalName(req.file.originalname), req.file.filename, now())
