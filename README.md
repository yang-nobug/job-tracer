...
| 邮箱与日程 | 右上角「日程」→ 连接 QQ 或 163 邮箱 → 扫描招聘候选；按封读取正文并由 AI 复核，复核通过后自动写入招聘日程 |
| 分享岗位 | 打开投递详情 →「分享到飞书」；群内会收到可直接复制的岗位文本，可粘贴回“招聘信息智能录入” |
...
当前版本支持个人 QQ 邮箱与 163 邮箱的安全连接、本地候选筛选、按需 AI 识别和复核通过后的自动招聘日程：

1. 在所选邮箱的网页设置中开启 IMAP/SMTP，并生成第三方客户端授权码
2. 点击页面右上角「日程」
3. 输入完整邮箱地址和授权码，点击「测试并保存」
4. 页面会显示收件箱总数及最近 5 封邮件的主题、发件人、时间和当前已读状态
5. 点击「扫描招聘邮件」；首次检查最近 100 封，之后只按 UID 增量读取新邮件，并展示可能属于面试、笔试、测评或 Offer 的候选。候选只按主题做宽松粗筛，发件人仅用于展示和后续 AI 上下文，因此自动转发到 163 的通知不会因发件人被改写而漏掉；包含无关邮件属于正常情况
...
QQ 邮箱固定使用 `imap.qq.com:993`，163 邮箱固定使用 `imap.163.com:993`，两者均使用 TLS，并以 IMAP `EXAMINE` 只读方式打开收件箱。连接测试和候选扫描只读取邮件信封；只有用户手动识别，或用户已开启自动处理并轮到该候选时，服务才按 MIME 结构读取该邮件的纯文本/HTML 正文部件，附件部件不会请求。任何流程都不会发送、删除、移动邮件或修改已读状态。候选由本地主题关键词规则筛选，发件人只作展示和 AI 上下文；主题、发件人、时间和命中标签保存在 SQLite，重复扫描不会重复入库，手动忽略后不会再次出现。当前一次只维护一个邮箱账户；切换 QQ/163 或邮箱地址会清理旧账户的同步游标、候选邮件和本机授权码，已确认日程仍会保留。
...
该功能通过飞书群“自定义机器人”的 Webhook，把某条投递的岗位信息发成纯文本消息，方便直接选中或长按复制。消息只包含公司、职位、地点、渠道、JD 链接和 JD 正文；不会发送投递进度链接、联系人、联系方式、简历或本地数据库中的其他内容。

1. 在目标飞书群中添加“自定义机器人”，复制其 Webhook 地址。
2. 在本机 `config.json` 根节点填入：`"feishu": { "webhookUrl": "https://open.feishu.cn/open-apis/bot/v2/hook/…" }`。
3. 重启 `start.bat`，打开任一投递详情，点击「分享到飞书」。
4. 在群消息中复制“招聘信息”正文，粘贴到新增投递的“招聘信息智能录入”中进行识别。

Webhook 是飞书提供的单向发送能力：Job Tracer 不读取群聊、历史消息或成员信息，卡片内容由用户手动复制回来识别。Webhook 仅由后端读取，不会返回到浏览器或写入运行日志；`config.json` 已被 Git 忽略，不要提交该文件。
...
AI 参数按 `ark.tasks` 中的任务配置：`applicationImport`、`jdParse`、`knowledgeExtract`、`answerGenerate`、`tutor`、`recordingReview`、`reviewAdvice`、`interviewPrepAgent`、`codeReading`、`resumeExtract`、`mailRecruitmentExtract` 和 `mailScheduleReview`。每项任务可单独设置 `enabled`、`model`、`outputMode`、`maxOutputTokens`、`temperature`、`timeoutMs` 和 `thinking`。旧的 `ark.recruitment` 仍兼容，等价于 `tasks.applicationImport`。页面右上角「AI 数据说明」可随时查看数据去向并在本机停用或重新启用各项能力。
...
上传 PDF 简历后，系统会在本机将最多 8 页按受限尺寸渲染为临时 JPEG，再逐页发送给 `resumeExtract` 配置的视觉模型提取文字；渲染图片在任务结束后立即删除。DOCX 简历仍只在本机解析 XML。提取在后台进行，页面会显示页级进度；扫描版 PDF 同样可使用此流程。
...
- `secrets/` — QQ / 163 邮箱授权码密文；解密密钥不放在项目或数据备份中
